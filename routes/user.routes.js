...
router.patch(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const user = await userService.updateUserDetails(userId, name, picture, null); // TODO: upload picture and get url
...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const user = await userService.updateUserDetails(userId, null, null, newPassword);
      res.status(200).end()
    } catch (err) {
      next(err);
    }
